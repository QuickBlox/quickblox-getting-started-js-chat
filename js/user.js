...
User.prototype.initGettingUsers = function () {
    ...
    self.getUsers().then(function(userList) {
        ...
        if(userList.length < 2) document.querySelector('.j-group__chat__description').innerHTML = 'There are no other registered users.';
...
User.prototype.getUsers = function () {
    var self = this,
        params = {
            ...
            per_page: CONSTANTS.LIMITS.USERS_PER_PAGE
        };
...
User.prototype.buildUserItem = function (user) {
    ...
    elem.addEventListener('click', function () {
        ...
        var selectedItems = self.userListConteiner.querySelectorAll('.selected');
        _.each(selectedItems, function (selectedItem) {
            if(selectedItem.classList.contains('disabled')) return false;
